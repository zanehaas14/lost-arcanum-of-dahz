...
CAMPAIGN_FACTIONS = [
    {"id": "cythranai", "label": "Cythranai", "uses": "wood-elf-realms"},
    {"id": "sontrailles", "label": "Sontrailles", "uses": "kingdom-of-bretonnia"},
]
ELF_ITEM_SOURCES = ["general", "wood-elf-realms", "high-elf-realms", "dark-elves"]
...
def apply_homebrew(by_id, faction_id):
    """Mutate by_id with the homebrew layer for faction_id. Returns (hb, touched)."""
...
def apply_cythranai_item_sources(units):
    """Standing rule: Cythranai characters draw items from all elf lists."""
    for u in units:
        if u.get("category") == "Characters" or u.get("magicAllowances"):
            if "itemSources" not in u:
                u["itemSources"] = list(ELF_ITEM_SOURCES)


def build_campaign_faction(campaign):
    """Clone a just-built official uses JSON, apply campaign homebrew, relabel."""
    uses_id = campaign["uses"]
    uses_path = os.path.join(FACTIONS_DIR, f"{uses_id}.json")
    if not os.path.exists(uses_path):
        if uses_id in FACTIONS:
            build_faction(uses_id)
        else:
            raise FileNotFoundError(f"missing uses faction {uses_id}")
    with open(uses_path, encoding="utf-8") as f:
        base = json.load(f)
    by_id = {u["id"]: copy.deepcopy(u) for u in base.get("units", [])}
    hb, touched = apply_homebrew(by_id, campaign["id"])
    units = list(by_id.values())
    units.extend(hb["units"])
    if campaign["id"] == "cythranai":
        apply_cythranai_item_sources(units)
    hb_total = touched + len(hb["units"])
    out = {
        "_comment": ("Auto-generated by tools/update-data.py. Do NOT hand-edit; "
                     f"add/edit custom units in data/homebrew/{campaign['id']}.json "
    ...
    with open(os.path.join(FACTIONS_DIR, f"{campaign['id']}.json"), "w",
              encoding="utf-8") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)
    return campaign["label"], len(units), hb_total


def write_index():
    """Campaign factions first (with uses), then official list alphabetical.
    Existing campaign JSON files are always included so a full official run
    does not drop them.
    """
    campaigns = []
    for c in CAMPAIGN_FACTIONS:
        if os.path.exists(os.path.join(FACTIONS_DIR, f"{c['id']}.json")):
            campaigns.append({"id": c["id"], "label": c["label"], "uses": c["uses"]})
    official = [{"id": fid, "label": lbl} for fid, lbl in FACTIONS.items()
                if os.path.exists(os.path.join(FACTIONS_DIR, f"{fid}.json"))]
    official.sort(key=lambda x: x["label"])
    index = campaigns + official
...
def build_faction(faction_id):
    ...
    hb, touched = apply_homebrew(by_id, faction_id)
...
def official_rule_key(out, name):
    """Match an official rules-index key: exact lowercased name, then the
    same name with a trailing parenthetical stripped (Armour Bane (1) ->
    armour bane). Official OWB keys do not carry the (N) suffix."""
    key = name.lower()
    if key in out:
        return key
    stripped = PAREN_SUFFIX.sub("", key).strip()
    if stripped and stripped in out:
        return stripped
    return None
...
def build_rules_index():
    ...
    # Fill-in sheet entries with empty strings must not become homebrew stubs,
    # including official names listed WITH parentheticals (Armour Bane (1)).
    hb = 0
    for name, val in load_homebrew_rules().items():
        desc = val.get("desc", "") if isinstance(val, dict) else val
        desc = desc.strip() if isinstance(desc, str) else (desc or "")
        official_key = official_rule_key(out, name)
        if official_key:
            # KEEP url/page; desc only if fill-in is non-empty; homebrew stays false
            if desc:
                out[official_key]["desc"] = desc
            out[official_key]["homebrew"] = False
        elif desc:
            key = name.lower()
            out[key] = {
                "page": "Homebrew",
            ...
            hb += 1
        # else: empty fill-in for an unknown name — skip, do not stub
...
def main():
    ...
    args = sys.argv[1:]
    campaign_by_id = {c["id"]: c for c in CAMPAIGN_FACTIONS}
    ...
    if args:
        ...
        for fid in args:
            if fid in FACTIONS:
                official_wanted.append(fid)
                for c in CAMPAIGN_FACTIONS:
                    if c["uses"] == fid and c["id"] not in seen_c:
                        campaigns_wanted.append(c)
                        seen_c.add(c["id"])
            elif fid in campaign_by_id:
                if fid not in seen_c:
                    campaigns_wanted.append(campaign_by_id[fid])
                    seen_c.add(fid)
            else:
                print(f"  ! unknown faction id: {fid} (skipping)")
    else:
        official_wanted = list(FACTIONS.keys())
        campaigns_wanted = list(CAMPAIGN_FACTIONS)

    for fid in official_wanted:
        try:
    ...
    for c in campaigns_wanted:
        try:
            label, n, hb = build_campaign_faction(c)
            extra = f" (+{hb} homebrew)" if hb else ""
            print(f"  ✓ {label} (uses {c['uses']}): {n} units{extra}")
        except Exception as e:  # noqa
